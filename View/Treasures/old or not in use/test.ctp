...
//debug($treasures);
 //the field below is for the HABTM test
	...
	/*
		...
		echo $this->Form->checkbox('bbm',array('div'=>false));

		echo $this->Form->checkbox('cfm',array('div'=>false,));

		echo $this->Form->checkbox('dmnh',array('div'=>false));

		echo $this->Form->checkbox('pim',array('div'=>false));

		echo $this->Form->checkbox('wg',array('div'=>false));
		
	*/
	        echo $this->Form->submit(__('Search', true), array('div' => true));	
			
			/* another way to make checkbox
		echo $this->form->input('Treasure.bbm', 
			array(
			'label'=>'BBM', 
			'type'=>'checkbox',
		//  'before' => '<label>WG</lablel>', 
			'div' => true,
			'hiddenField' => false
			));
			*/
