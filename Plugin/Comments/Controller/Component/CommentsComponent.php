 ...
 */
	public function callback_fetchDataTree($options) {
		...
		//the settings above are conditions, such as comment approved, spam, etc.
		$settings += array('order' => array('Comment.lft' => 'asc'));
		$paginate = $settings;
		$paginate['limit'] = 10;
		
		//use this to find where we are (usergals or treasures, couldn't get $this->here or other things to work
		$where=explode('/',$_SERVER['REQUEST_URI']);
//		debug($where);
		/*sj -added the fields to get UserModel email as well as other stuff,
		//if you want to see the default behavior, just comment out the 'fields' key assignment below
		and debug($data) - which is the same as commentsData on the view!
		*/

		if ($where[1]=='treasures') $paginate['fields'] = array('Comment.*','Treasure.id','UserModel.id','UserModel.username','UserModel.slug','UserModel.email');
		if ($where[1]=='usergals') $paginate['fields'] = array('Comment.*','Usergal.id','UserModel.id','UserModel.username','UserModel.slug','UserModel.email');
 ...
 */
	public function callback_fetchDataThreaded($options) {
		...
		$paginate['fields'] = array('Comment.*','Treasure.id','UserModel.id','UserModel.username','UserModel.slug','UserModel.email');
 ...
 */
	...
	public function callback_add($modelId, $commentId, $displayType, $data = array()) {
		if (!empty($this->Controller->data)) {
			...
			//seth added the following IF to fix their sloppy code
			if (!empty($this->Controller->data['Comment']['body'])) {
			...
			if (!is_null($result)) {
				if ($result) {
					...
					//here is the redirect to disable for testing
