...
<div class="comments">
	...
	<?php
		if ($allowAddByAuth):
			...
			<?php
				echo sprintf(__d('comments', 'If you want to post comments, you need to '.$this->Html->link('login',array('controller'=>'users','plugin'=>'users','action'=>'login')) .' first.'), $this->Html->link(__('comments', 'login'), array('plugin'=>'users','controller' => 'users', 'action' => 'login', 'prefix' => $adminRoute, $adminRoute => false)));
		endif;
